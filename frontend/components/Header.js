...
const Header = () => {
  ...
  const { data: themeData, isLoading: isLoadingTheme } =
  ...
  return (
    <header
      className={classNames("shadow-sm border-b w-full", {
        "bg-white border-gray-200": theme === "light",
        "bg-[#181A20] border-gray-700": theme === "dark",
      })}
    >
            ...
            className={classNames(
              "text-sm font-medium px-2 pb-1 border-b-2 transition-colors duration-200 whitespace-nowrap",
              {
                "border-indigo-600 text-indigo-600":
                  activeTab === id && theme === "light",
                "border-purple-400 text-purple-400":
                  activeTab === id && theme === "dark",
